...
def is_valid_plum_address(address, network='mainnet'):
    ...
    plum_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Plum address
        return False

    if (address_version != plum_version):
        return False
...
# shims 'til we can fix the plumd side
def SHIM_serialise_for_plumd(sentinel_hex):
    from models import PLUMD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = PLUMD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in plumd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # plumd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    plumd_hex = serialise(obj)
    return plumd_hex


# shims 'til we can fix the plumd side
def SHIM_deserialise_from_plumd(plumd_hex):
    from models import PLUMD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(plumd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('plum.conf')
