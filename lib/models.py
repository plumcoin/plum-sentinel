...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, plumd):
        golist = plumd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_plumd(plumd, item)
    ...
    @classmethod
    def import_gobject_from_plumd(self, plumd, rec):
        import decimal
        import plumlib
        import inflection
        ...
        # shim/plumd conversion
        object_hex = plumlib.SHIM_deserialise_from_plumd(object_hex)
        objects = plumlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from plumd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from plumd, with every run
        ...
        # get/create, then sync payment amounts, etc. from plumd - Plumd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(plumd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from plumd! %s" % e)
            govobj.vote_delete(plumd)
            return (govobj, None)
    ...
    def vote_delete(self, plumd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(plumd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, plumd, signal, outcome):
        import plumlib
        ...
        output = plumd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = plumlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(plumd, signal)

    def sync_network_vote(self, plumd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = plumd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = PLUMD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 plum addr, non-multisig
            if not plumlib.is_valid_plum_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Plum address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (plumlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/PlumDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import plumlib
        obj_data = plumlib.SHIM_serialise_for_plumd(self.serialise())
    ...
    def prepare(self, plumd):
        try:
            object_hash = plumd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = PLUMD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not plumlib.is_valid_plum_address(addr, config.network):
    ...
    def hash(self):
        import plumlib
        return plumlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = PLUMD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, plumd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - plumd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, plumd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - plumd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, plumd):
        now = int(time.time())
        return (self.created_at < (now - plumd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, plumd):
        if self.is_expired(plumd):
            return False

        return True

    def is_deletable(self, plumd):
        if self.is_expired(plumd):
            return True
