...
def has_plum_conf():
    ...
    valid_plum_conf = False

    # ensure plum_conf exists & readable
    #
    # if not, print a message stating that Plum Core must be installed and
    # configured, including JSONRPC access in plum.conf
    try:
        f = io.open(config.plum_conf)
        valid_plum_conf = True
    except IOError as e:
        print(e)

    return valid_plum_conf
...
def main():
    ...
    if not has_plum_conf():
        print("PlumCore must be installed and configured, including JSONRPC access in plum.conf")
        sys.exit(1)
