...
class PlumDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some plumd methods
    ...
    @classmethod
    def from_plum_conf(self, plum_dot_conf):
        from plum_config import PlumConfig
        config_text = PlumConfig.slurp_config_file(plum_dot_conf)
        creds = PlumConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = plumlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = plumlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = plumlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_plumd_proto_version_with_sentinel_ping)
