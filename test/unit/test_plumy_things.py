...
def test_valid_plum_address():
    from plumlib import is_valid_plum_address

    main = valid_plum_address()
    test = valid_plum_address('testnet')

    assert is_valid_plum_address(main) is True
    assert is_valid_plum_address(main, 'mainnet') is True
    assert is_valid_plum_address(main, 'testnet') is False

    assert is_valid_plum_address(test) is False
    assert is_valid_plum_address(test, 'mainnet') is False
    assert is_valid_plum_address(test, 'testnet') is True


def test_invalid_plum_address():
    from plumlib import is_valid_plum_address

    main = invalid_plum_address()
    test = invalid_plum_address('testnet')

    assert is_valid_plum_address(main) is False
    assert is_valid_plum_address(main, 'mainnet') is False
    assert is_valid_plum_address(main, 'testnet') is False

    assert is_valid_plum_address(test) is False
    assert is_valid_plum_address(test, 'mainnet') is False
    assert is_valid_plum_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from plumlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % plumlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(plumlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(plumlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(plumlib.blocks_to_seconds(16616)) == 2612035
